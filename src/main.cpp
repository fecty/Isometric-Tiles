...
Color fgColor = {255 - bgColor.r, 255 - bgColor.g, 255 - bgColor.b, 255}; // Opposite of background color
...
void handleEvents();
void drawGame();
void drawTile(int x, int y, Vector2 startPos, int gridSize, float altitude, bool showOutline = false);
...
void handleEvents()
{
    ...
    gridSize = static_cast<int>(Clamp(gridSize, 1.f, float(MAX_GRID_SIZE)));
...
void drawGame()
{
    ...
    Vector2 startPos = {(w - tileTex.width) / 2.f, // to center a unit tile to its center
                        h / 2.f};
...
void drawTile(int x, int y, Vector2 startPos, int gridSize, float altitude, bool showOutline)
{

    Vector2 isoCoords = transform({x * tileTex.width, y * tileTex.height}); // isometric transformation
    ...
    if (showOutline)
        DrawRectangleLines(isoCoords.x, isoCoords.y, tileTex.width, tileImg.height, RED); // Show outline of tiles

    DrawTexture(tileTex, isoCoords.x, isoCoords.y, fgColor);
}
